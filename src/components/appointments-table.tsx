...
export function AppointmentsTable({ appointments, onCancel }: AppointmentsTableProps) {
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'confirmed': return 'bg-green-600/20 text-green-300 border-green-600/30'
        ...
        <TableBody>
          {appointments.map((appointment) => (
            <TableRow key={appointment.id} className="border-slate-800 hover:bg-slate-800/50">
              <TableCell className="text-white font-medium">{appointment.patientName}</TableCell>
              <TableCell className="text-slate-300">{appointment.phoneNumber}</TableCell>
              <TableCell className="text-slate-300">{appointment.doctorName}</TableCell>
              <TableCell className="text-slate-300">{appointment.dateTime}</TableCell>
              <TableCell>
                <Badge className={`capitalize border ${getStatusColor(appointment.status)}`} variant="outline">
                  {appointment.status}
                </Badge>
              </TableCell>
              <TableCell>
